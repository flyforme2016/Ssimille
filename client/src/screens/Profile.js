...
const Stats = ({count , item}) => {
  ...
  <View style = {{marginLeft : 25, marginRight : 25}}>
    <Text size ='20px' bold alignItems = 'center'>{count}</Text>
    <Text size ='10px'  >{item}</Text>
  </View>
  )
}
...
const Profile = ({
  ...
  return (
    <Container>
      ...
      <Image source={AVARTA}/>
      <Text size = '20px' bold style={{marginTop : 16}}>이명희</Text>
      <Text  size = '10px' >혼네 (HONNE) - WHAT WOULD YOU DO? (Feat. 핑크 스웨츠 Pink Sweat$) </Text>
      <Text size = '15px' style = {{marginTop :30}}>안녕하세요 인천에 살고있는 26살 이명희 입니다!</Text>
      <StatsBox>
      <Stats count = '게시글' item = '2개'/>
      <View style= {{width : 2, height : 30, backgroundColor : '#9b59b6'}}></View>
