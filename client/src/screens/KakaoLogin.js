...
const KakaoLogin = ({navigation: {navigate, replace}}) => {
  ...
  const parseAuthCode = async url => {
      ...
      if(startIndex !== -1){
        ...
        try {
          await axios
            .post(apiBaseUrl+'/kakao/oauth/callback', {
              code: authCode,
            })
            .then(async res => {
              //스포티파이 연동
  ...
  return (
    <View style={{flex: 1}}>
      <WebView
        ...
        source={{
          uri: `https://kauth.kakao.com/oauth/authorize?response_type=code&client_id=1c1252b4d425329642c458690fe99854&redirect_uri=${apiBaseUrl}/kakao/oauth/callback`,
        }}
